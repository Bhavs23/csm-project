...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user is None or not user.check_password(form.password.data):
            ...
            LOG.warning('WARNING: Login Unsuccessful — invalid username or password.')
...
def authorized():
    ...
    if request.args.get('code'):
        ...
        if "error" in result:
            LOG.error('ERROR: Token acquisition failed...')
            return render_template("auth_error.html", result=result)

        session["user"] = result.get("id_token_claims")

        # Always map MS login to admin account for this project
        user = User.query.filter_by(username="admin").first()
        login_user(user)
        _save_cache(cache)

        LOG.info('INFO: Azure AD user logged in.')
